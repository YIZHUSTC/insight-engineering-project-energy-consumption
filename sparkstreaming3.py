...
def getRecent(tuples):
    
    
    return (tuples[0], tuples[1][0], tuples[1][1], tuples[1][2], tuples[1][3], tuples[1][4], tuples[1][5], tuples[1][6], tuples[1][7], tuples[1][8], tuples[1][9], tuples[1][10], tuples[1][11], tuples[1][12])
...
def main():
    ...
    #Recent_data=Processed_data.map(getRecent).sortBy(_._3, ascending = false)
    Recent_data=Processed_data.map(getRecent)
    ...
    Recent_data.foreachRDD(lambda rdd: rdd.foreachPartition(sendCassandrarecent))
